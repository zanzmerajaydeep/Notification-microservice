...
public class SMSService {
	...
	public List<ResponseEntity<String>> callSMSService() {
		...
		for (Hospital h : hospitalList) {
			...
			String accountSid = sa.getAccount_sid();
			String authToken = sa.getAuth_token();
			String smsfrom=sa.getPhoneNo();
			
			smsData.put("smsto", contactNo);
			smsData.put("subject", subject);
			smsData.put("body", body);
			smsData.put("account_sid", accountSid);
			smsData.put("auth_token", authToken);
	...
	//method call SMS Service By ContactNo
	@SuppressWarnings("null")
	public ResponseEntity<String> callSMSServiceByContactNo(String contNo) {
		...
		String url = "http://localhost:8888/smsmicroservice/sms/sendsms";
			...
			try {
				...
				return restTemplet.postForEntity(url, smsData, String.class);
			} catch (Exception e) {
				e.printStackTrace();
